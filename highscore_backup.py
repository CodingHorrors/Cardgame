            ...
            if setting_selection == "0":
                ...
                with open(p, mode='a+', newline='') as score_file:
                    ...
                    if os.path.getsize(p) == 0:
                        ...
                        new_frame = init_panda.append(append_frame, ignore_index=True)
                ...
                with open(p, mode='a+', newline='') as score_file:
                    ...
                    try:
                        ...
                        continue
                    name_list = list(score_read["Name"])
                    name_list_disp, display_count = new_display(name_list)
                    del_choice = int(input(f"Select Name to delete it.\n{name_list_disp}\n>"))
                    new_frame = score_read.drop(del_choice, axis=0)
                    range_list = [] #list of the label indices because indices are
                    for i in range(len(name_list)-1): #not updated after deletion
                        range_list.append(i)
                    new_frame.index = range_list
                    with open(p, mode='w', newline='') as new_score_file:
                        new_frame.to_csv(new_score_file)
                    print(new_frame)
